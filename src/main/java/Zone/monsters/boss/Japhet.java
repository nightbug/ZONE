...
public class Japhet extends CustomMonster
{
    public static final String ID = makeID(Japhet.class.getSimpleName());
    ...
    public Japhet(final float x, final float y) {
        super(Japhet.NAME, ID, HP, -5.0F, 0, 280.0f, 340.0f, null, x, y);
        this.animation = new BetterSpriterAnimation("zoneResources/images/monsters/boss/japhet/Spriter/japhetAnimation.scml");
        ...
        Player.PlayerListener listener = new JaphetListener(this);
    ...
    protected void getMove(final int num) {
        if (!usedMinute) {
            this.setMove(Japhet.MOVES[0], MINUTE_HAND, Intent.ATTACK, damage.get(0).base);
            usedMinute = true;
        }  else if (!usedHour) {
            this.setMove(Japhet.MOVES[1], HOUR_HAND, Intent.ATTACK, damage.get(1).base);
            usedHour = true;
        } else {
            this.setMove(Japhet.MOVES[2], SWEEP_HAND, Intent.ATTACK, damage.get(2).base, SWEEP_HAND_HITS, true);
            usedMinute = false;
            usedHour = false;
        }
    }
    ...
    static {
        ...
        NAME = Japhet.monsterStrings.NAME;
        MOVES = Japhet.monsterStrings.MOVES;
        DIALOG = Japhet.monsterStrings.DIALOG;
    }
    ...
    public class JaphetListener implements Player.PlayerListener {

        private Japhet character;

        public JaphetListener(Japhet character) {
