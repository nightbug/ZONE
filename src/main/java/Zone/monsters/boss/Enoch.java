...
public class Enoch extends CustomMonster
{
    public static final String ID = makeID(Enoch.class.getSimpleName());
    ...
    public Enoch(final float x, final float y) {
        super(Enoch.NAME, ID, HP, -5.0F, 0, 280.0f, 340.0f, null, x, y);
        this.animation = new BetterSpriterAnimation("zoneResources/images/monsters/boss/enoch/Spriter/enochAnimation.scml");
        ...
        Player.PlayerListener listener = new EnochListener(this);
    ...
    protected void getMove(final int num) {
        if (!usedMinute) {
            this.setMove(Enoch.MOVES[0], MINUTE_HAND, Intent.ATTACK, damage.get(0).base);
            usedMinute = true;
        }  else if (!usedHour) {
            this.setMove(Enoch.MOVES[1], HOUR_HAND, Intent.ATTACK, damage.get(1).base);
            usedHour = true;
        } else {
            this.setMove(Enoch.MOVES[2], SWEEP_HAND, Intent.ATTACK, damage.get(2).base, SWEEP_HAND_HITS, true);
            usedMinute = false;
            usedHour = false;
        }
    }
    ...
    static {
        ...
        NAME = Enoch.monsterStrings.NAME;
        MOVES = Enoch.monsterStrings.MOVES;
        DIALOG = Enoch.monsterStrings.DIALOG;
    }
    ...
    public class EnochListener implements Player.PlayerListener {

        private Enoch character;

        public EnochListener(Enoch character) {
